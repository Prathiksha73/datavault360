...
const DoctorDashboard = () => {
    ...
    const handleInvitePatient = async (e) => {
        ...
        try {
            ...
            toast.success('Patient Invitiation Sent!');
            setShowModal(false);
            setFormData({ email: '', date_of_birth: '', phone_number: '', address: '' });
        } catch (error) {
            console.error(error);
        ...
        <Layout role="DOCTOR">
            ...
            {inviteLink && (
                <div className="mb-8 p-6 bg-green-50 border border-green-200 rounded-xl">
                    ...
                    <p className="text-green-700 mb-4">You can share this link with the patient directly if needed:</p>
                    <div className="bg-white p-4 rounded-lg border border-green-100 font-mono text-sm mb-4 break-all">
                        {inviteLink}
                    </div>
                    <button onClick={() => { navigator.clipboard.writeText(inviteLink); toast.success('Copied!'); }} className="text-indigo-600 font-medium hover:underline">Copy Link</button>
                    <button onClick={() => setInviteLink(null)} className="ml-4 text-gray-500 hover:text-gray-700">Dismiss</button>
                </div>
            )}
                            ...
                            >
                                <div className="flex items-center justify-between mb-4">
                                    <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center font-bold text-lg">
                                        {patient.user.first_name ? patient.user.first_name[0].toUpperCase() : patient.user.username[0].toUpperCase()}
                                    </div>
                                    <span className="text-xs font-semibold px-2 py-1 bg-green-100 text-green-700 rounded-full">Active</span>
                                </div>
                                <h3 className="text-lg font-bold text-gray-800">
                                    {patient.user.first_name && patient.user.last_name ? `${patient.user.first_name} ${patient.user.last_name}` : patient.user.username}
                                </h3>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]">
                    ...
                    >
                        <h2 className="text-2xl font-bold mb-6">Invite New Patient</h2>
                        <form onSubmit={handleInvitePatient} className="space-y-4">
                            ...
                            <div className="flex space-x-3 mt-6">
                                ...
                                <button type="submit" className="flex-1 py-3 text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-md">Send Invite</button>
                            </div>
                        </form>
                    </motion.div>
                </div>
